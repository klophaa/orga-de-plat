// Orga de plat
// Header simplified to avoid encoding issues during cloud builds.
...
const ALLOWED_EMAILS = {
  "theo@orgadeplat.fr":   { displayName: "ThÃ©o",   avatar: "ðŸ§‘â€ðŸ³", color: "#4f86c6" },
  "elodie@orgadeplat.fr": { displayName: "Elodie", avatar: "ðŸ‘©â€ðŸ³", color: "#6bab8a" },
};
...
function CropTool({ src, onDone, onCancel, T }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16,alignItems:"center"}}>
      ...
      <div style={{width:"100%"}}>
        <div style={{fontSize:11,color:T.textMuted,marginBottom:4,fontWeight:600}}>ðŸ” Zoom</div>
    ...
    <div style={{position:"relative",overflow:"hidden",borderRadius:14}}>
      ...
      {swipeHint==='right'&&<div style={{position:"absolute",inset:0,background:"linear-gradient(90deg,#4f86c6,#6bab8a)",display:"flex",alignItems:"center",paddingLeft:16,borderRadius:18,zIndex:0}}>
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,transition:"transform 0.1s"}}>
          <span style={{fontSize:Math.min(16+Math.abs(swipeX)/8,32),transition:"font-size 0.1s",filter:"drop-shadow(0 2px 4px rgba(0,0,0,0.2))"}}>ðŸ“…</span>
      ...
      >
        ...
        <div style={{position:"relative",width:"100%",aspectRatio:"1/1",background:T.accentLight,overflow:"hidden"}}>
          {thumb
            ...
            : <div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:48}}>ðŸ½ï¸</div>
          }
        ...
        <div style={{padding:"10px 10px 12px"}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <div style={{display:"flex",alignItems:"center",gap:3,flex:1}}>
              <span style={{fontSize:11}}>ðŸ½ï¸</span>
              <div style={{display:"flex",gap:1}}>{Array.from({length:3}).map((_,i)=><span key={i} style={{fontSize:10,opacity:i<(dish.dishesRating||0)?1:0.15,color:T.accent}}>â—</span>)}</div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:3,flex:1}}>
              <span style={{fontSize:11}}>â±ï¸</span>
              <div style={{display:"flex",gap:1}}>{Array.from({length:3}).map((_,i)=><span key={i} style={{fontSize:10,opacity:i<(dish.timeRating||0)?1:0.15,color:T.green}}>â—</span>)}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{display:"flex",background:T.bg,borderRadius:12,padding:3,border:`1.5px solid ${T.cardBorder}`}}>
        {[["custom","âœ‹ Choix manuel"],["auto","âš™ï¸ Filtre auto"]].map(([m,label])=>(
      ...
      {wheelMode==="auto"&&<div style={{...s.card}}>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div><label style={s.label}>{"â±ï¸ Temps max : "+randomFilters.maxTime+"/3"}</label><input type="range" min={1} max={3} value={randomFilters.maxTime} onChange={e=>setRandomFilters(f=>({...f,maxTime:+e.target.value}))} style={{width:"100%",accentColor:T.green}}/></div>
          <div><label style={s.label}>{"ðŸ«§ Vaisselle max : "+randomFilters.maxDishes+"/3"}</label><input type="range" min={1} max={3} value={randomFilters.maxDishes} onChange={e=>setRandomFilters(f=>({...f,maxDishes:+e.target.value}))} style={{width:"100%",accentColor:"#4aa8b8"}}/></div>
        </div>
      </div>}
      ...
      {wheelDishes.length===0&&<div style={{textAlign:"center",color:T.textLight,fontSize:14,padding:24}}>
        {wheelMode==="auto"?"Aucun plat ne correspond aux filtres ðŸ˜…":"Ajoute des plats pour tourner la roue ðŸ‘†"}
      </div>}
        ...
        <div style={{position:"relative",display:"flex",justifyContent:"center",width:"100%"}}>
          ...
          <svg width="260" height="260" viewBox="-130 -130 260 260"
            ...
            <g onClick={()=>!wheelSpinning&&spinWheel(wheelDishes)} style={{cursor:wheelSpinning?"default":"pointer"}}>
              ...
              <text x="0" y="6" textAnchor="middle" fontSize="15">{wheelSpinning?"ðŸŒ€":"ðŸŽ°"}</text>
            </g>
          </svg>
        </div>
        {wheelSpinning&&<div style={{fontSize:13,color:T.textMuted,fontWeight:600,animation:"pulse 1s infinite"}}>ðŸŒ€ En train de tournerâ€¦</div>}
...
function WheelFreeInput({ onAdd, T, s }) {
  return (
    <div style={{display:"flex",gap:8,marginTop:10}}>
      <input
        placeholder={"âœï¸ Ajouter un mot libre..."}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(10,20,35,0.55)",zIndex:1000,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:T.card,borderRadius:"22px 22px 0 0",padding:"0 24px 28px",width:"100%",maxWidth:480,maxHeight:`calc(92vh - ${kbOffset}px)`,overflowY:"auto",boxShadow:`0 -20px 60px rgba(0,0,0,0.25)`,marginBottom:kbOffset,transition:"margin-bottom 0.2s,max-height 0.2s"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:14}}>
          <input
            placeholder={"âœï¸ Ã‰crire un plat libre..."}
        ...
        <input
          placeholder={"ðŸ” Rechercher une fiche..."}
          ...
          {filtered.map(d=>(
            <button key={d.id} onClick={()=>handleSelect(d)}
              ...
              <div style={{width:44,height:44,borderRadius:10,background:T.accentLight,flexShrink:0,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>
                {(d.thumbnail||d.photo)?<img src={d.thumbnail||d.photo} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"ðŸ½ï¸"}
              </div>
...
export default function App() {
  ...
  // â”€â”€ Ã‰tats onglets privÃ©s Ã‰lodie â”€â”€
  ...
  const [wheelMode, setWheelMode] = useState("custom"); // "auto" | "custom"
  ...
  const filteredDishes = useMemo(() => {
    ...
    // Tri rÃ©cents toujours appliquÃ© en base
  ...
  // â”€â”€ Export / Restauration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const restoreData = async (file) => {
    ...
    try {
      ...
      if (!backup.version || !backup.dishes) throw new Error("Fichier invalide");
      // Restaurer plats
      let batch = writeBatch(db); let count = 0;
      for (const d of (backup.dishes||[])) {
        const {id,...data} = d;
        batch.set(doc(db,"dishes",id), data);
        count++;
        if (count%400===0) { await batch.commit(); batch=writeBatch(db); }
      }
      // Restaurer planning
      for (const [key,plan] of Object.entries(backup.weekPlans||{})) {
        batch.set(doc(db,"weekPlans",key), plan);
        count++;
        if (count%400===0) { await batch.commit(); batch=writeBatch(db); }
      }
      // Restaurer idÃ©es
      for (const d of (backup.ideas||[])) {
        const {id,...data} = d;
        batch.set(doc(db,"ideas",id), data);
        count++;
        if (count%400===0) { await batch.commit(); batch=writeBatch(db); }
      }
      // Restaurer plats Ã‰lodie
      for (const d of (backup.elodieDishes||[])) {
        const {id,...data} = d;
        batch.set(doc(db,"elodieDishes",id), data);
        count++;
        if (count%400===0) { await batch.commit(); batch=writeBatch(db); }
      }
      await batch.commit();
      setRestoreMsg({ok:true, msg:`âœ… Restauration rÃ©ussie â€” ${count} Ã©lÃ©ments importÃ©s`});
    } catch(e) {
      setRestoreMsg({ok:false, msg:"âŒ Erreur : "+e.message});
    }
    setRestoring(false);
  };
  ...
  // â”€â”€ DonnÃ©es calculÃ©es onglets Ã‰lodie â”€â”€
  const isElodie = currentUser === "Elodie";
  // Source de plats active pour planifier (dÃ©pend du toggle)
  ...
  const ElodiePlanSlot = ({slot, isWeekend}) => {
    ...
    return (
      <div onDragOver={e=>{e.preventDefault();setOver(true);}} onDragLeave={()=>setOver(false)} onDrop={e=>{e.preventDefault();setOver(false);handleElodieDrop(slot);}}
        ...
        <div style={{fontSize:10,fontWeight:700,color:isWeekend?T.weekendHeader:T.weekdayHeader,textTransform:"uppercase",letterSpacing:0.5,marginBottom:6}}>{meal==="midi"?"â˜€ï¸ Midi":"ðŸŒ™ Soir"}</div>
        {display?(
          <div draggable onDragStart={()=>setElodieDragItem({slot,dish:entry})}>
            <div style={{width:"100%",aspectRatio:"4/3",borderRadius:9,background:T.accentLight,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,marginBottom:6}}>
              {(display.photo||display.thumbnail)?<img src={display.thumbnail||display.photo} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"ðŸ½ï¸"}
            </div>
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              <div style={{flex:1,fontSize:12,fontWeight:700,color:T.text,lineHeight:1.3,wordBreak:"break-word"}}>{display.name}</div>
              <button onClick={()=>setConfirmAction({title:"Retirer du planning ?",message:`"${display.name}" sera retirÃ© de ce crÃ©neau.`,onConfirm:()=>{removeElodieFromPlan(slot);setConfirmAction(null);}})} style={{...s.iconBtn,fontSize:15,flexShrink:0}}>{"Ã—"}</button>
            </div>
          </div>
  ...
  const TABS=[
    {id:"dishes",icon:"ðŸ¥˜",label:"Plats"},
    {id:"plan",icon:"ðŸ“…",label:"Planning"},
    {id:"ideas",icon:"ðŸ’¡",label:"IdÃ©es"},
    {id:"random",icon:"ðŸª„",label:"AlÃ©atoire"},
    {id:"tools",icon:"ðŸ”§",label:"Outils"},
    {id:"suivi",icon:"ðŸ”Ž",label:"Suivi"},
    {id:"elodieDishes",icon:"ðŸ‘©â€ðŸ³",label:"Plats Ã‰lodie"},
    ...(isElodie ? [
      {id:"elodiePlan",icon:"ðŸ’œ",label:"Plan Ã‰lodie"},
    ] : []),
  ];
  ...
  const CompactDishCard=({dish,onSelect})=>{
    ...
    return (
      <div onClick={()=>onSelect(dish)} style={{...s.card,display:"flex",gap:12,alignItems:"center",cursor:"pointer"}}>
        <div style={{width:44,height:44,borderRadius:10,background:T.accentLight,flexShrink:0,overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>
          {thumb?<img src={thumb} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"ðŸ½ï¸"}
        </div>
      ...
      <div onDragOver={e=>{e.preventDefault();setOver(true);}} onDragLeave={()=>setOver(false)} onDrop={e=>{e.preventDefault();setOver(false);handleDrop(slot);}}
        ...
        {display?(
          <div draggable onDragStart={()=>setDragItem({slot,dish:entry})}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              <div style={{flex:1,fontSize:12,fontWeight:700,color:T.text,lineHeight:1.3,wordBreak:"break-word"}}>{display.name}</div>
              <button onClick={()=>setConfirmAction({title:"Retirer du planning ?",message:`"${display.name}" sera retirÃ© de ce crÃ©neau.`,onConfirm:()=>{removeFromPlan(slot);setConfirmAction(null);}})} style={{...s.iconBtn,fontSize:15,flexShrink:0}}>{"Ã—"}</button>
            </div>
          </div>
  ...
  // PlanSlotNext â€” pour semaine pro et W-E pro (nextWeekPlan)
  const PlanSlotNext=({slot,isWeekend})=>{
    ...
    const assignNextDish = async (dish, slots) => {
      ...
      logActivity(`a planifiÃ© "${dish.name}" (semaine pro)`);
    };
    return (
      <div onDragOver={e=>{e.preventDefault();setOver(true);}} onDragLeave={()=>setOver(false)} onDrop={e=>{e.preventDefault();setOver(false);}}
        ...
        {display?(
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              <div style={{flex:1,fontSize:12,fontWeight:700,color:T.text,lineHeight:1.3,wordBreak:"break-word"}}>{display.name}</div>
              <button onClick={()=>setConfirmAction({title:"Retirer du planning ?",message:`"${display.name}" sera retirÃ© de ce crÃ©neau.`,onConfirm:()=>{removeFromNextPlan(slot);setConfirmAction(null);}})} style={{...s.iconBtn,fontSize:15,flexShrink:0}}>{"Ã—"}</button>
            </div>
          </div>
  ...
  const DishForm=({initial,onSave,onCancel})=>{
    ...
    return (
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:12,alignItems:"center"}}>
            <div style={{width:60,height:60,borderRadius:12,background:T.accentLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,overflow:"hidden",flexShrink:0}}>
              {form.thumbnail?<img src={form.thumbnail} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"ðŸ½ï¸"}
            </div>
            <label style={{...s.ghost,fontSize:13,cursor:"pointer",padding:"8px 14px"}}>ðŸ“· Choisir<input type="file" accept="image/*" style={{display:"none"}} onChange={handleFile}/></label>
        ...
        <div><label style={s.label}>ðŸ½ï¸ Vaisselle</label><StarRating icon="ðŸ«§" value={form.dishesRating} max={3} onChange={v=>set("dishesRating",v)} color={T.accent} size={22}/></div>
        <div><label style={s.label}>â±ï¸ Temps de prÃ©paration</label><StarRating icon="â±ï¸" value={form.timeRating} max={3} onChange={v=>set("timeRating",v)} color={T.green} size={22}/></div>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:12,alignItems:"center"}}>
            <div style={{width:52,height:52,borderRadius:10,background:T.accentLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,overflow:"hidden"}}>
              {form.thumbnail?<img src={form.thumbnail} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"ðŸ’¡"}
            </div>
            <label style={{...s.ghost,fontSize:13,cursor:"pointer",padding:"8px 14px"}}>ðŸ“· Ajouter<input type="file" accept="image/*" style={{display:"none"}} onChange={handleFile}/></label>
            {form.thumbnail&&<button onClick={()=>{set("thumbnail",null);set("photo",null);}} style={{...s.iconBtn,color:T.danger}}>âœ•</button>}
          </div>
        </div>
        <div style={{display:"flex",gap:8}}>
    ...
    <div style={s.app}>
      ...
      <div style={{flex:1,padding:16,paddingBottom:36}}>
        ...
        {tab==="dishes"&&<div>
          ...
          <div style={{display:"flex",gap:6,marginBottom:12,alignItems:"center",flexWrap:"wrap"}}>
            <div style={{display:"flex",gap:4,alignItems:"center"}}>
              ...
              {[{id:"dishes",label:"ðŸ«§ Vaisselle"},{id:"time",label:"â±ï¸ Temps"}].map(opt=>(
              ...
              {sortBy&&<button onClick={()=>setSortBy(null)} style={{padding:"4px 7px",borderRadius:8,border:`1.5px solid ${T.inputBorder}`,background:"transparent",color:T.textMuted,fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>âœ•</button>}
            </div>
          ...
          {/* Grille 2 colonnes */}
          {filteredDishes.length===0&&<div style={{textAlign:"center",color:T.textLight,padding:"40px 0"}}>Aucun plat trouvÃ© ðŸ½ï¸</div>}
          <style>{`
        ...
        {tab==="plan"&&<div>
          ...
          <div style={{display:"flex",background:T.segBg,borderRadius:10,padding:2,gap:2,marginBottom:14}}>
            {[{id:"own",label:isElodie?"ðŸ‘©â€ðŸ³ Mes plats":"ðŸ§‘â€ðŸ³ Mes plats"},{id:"other",label:isElodie?"ðŸ§‘â€ðŸ³ Plats ThÃ©o":"ðŸ‘©â€ðŸ³ Plats Ã‰lodie"}].map(opt=>(
          ...
          {pastWeeks.length>0&&<div style={{marginTop:24}}><div style={{fontWeight:700,color:T.textMuted,fontSize:11,textTransform:"uppercase",letterSpacing:0.5,marginBottom:10}}>ðŸ“š Semaines passÃ©es</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{pastWeeks.map(wk=><button key={wk} onClick={()=>setHistoryWeek(wk)} style={{...s.card,padding:"10px 14px",textAlign:"left",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontWeight:600,color:T.text,fontSize:13}}>Semaine du {wk}</div><div style={{fontSize:11,color:T.textMuted}}>{Object.values(weekPlans[wk]||{}).filter(Boolean).length} repas</div></button>)}</div></div>}
        </div>}
        ...
        {tab==="ideas"&&<div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {ideas.length===0&&<div style={{textAlign:"center",color:T.textLight,padding:"40px 0"}}>Aucune idÃ©e pour l'instant ðŸ’¡</div>}
            {ideas.map(idea=><div key={idea.id} style={{...s.card,opacity:idea.tested?0.6:1}}>
              <div style={{display:"flex",gap:10}}>
                <div style={{width:50,height:50,borderRadius:10,background:T.greenLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,overflow:"hidden",flexShrink:0}}>
                  {(idea.thumbnail||idea.photo)?<img src={idea.thumbnail||idea.photo} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"ðŸ’¡"}
                </div>
                <div style={{flex:1}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    <div style={{fontWeight:700,fontSize:15,color:T.text}}>{idea.title||"Sans titre"}</div>
                    <div style={{display:"flex",gap:2}}>
                      <button onClick={()=>setEditIdea(idea)} style={s.iconBtn}>âœï¸</button>
                      <button onClick={()=>deleteDoc(doc(db,"ideas",idea.id))} style={s.iconBtn}>ðŸ—‘ï¸</button>
                    </div>
                  </div>
                  ...
                  {(idea.links||[]).filter(l=>l.url).map((l,i)=><a key={i} href={l.url} target="_blank" rel="noreferrer" style={{fontSize:12,color:T.accent,display:"block",marginTop:4}}>ðŸ”— {l.label||l.url}</a>)}
                  {!idea.links&&idea.link&&<a href={idea.link} target="_blank" rel="noreferrer" style={{fontSize:12,color:T.accent,display:"block",marginTop:4}}>ðŸ”— Voir la recette</a>}
                  <div style={{display:"flex",gap:8,marginTop:10}}>
        ...
        {/* â•â• STATS â•â• */}
        {tab==="suivi"&&<div>
          <div style={{fontWeight:800,fontSize:16,color:T.text,marginBottom:16}}>ðŸ“Š Statistiques</div>
          {/* STREAK */}
          {streakCount>0&&<div style={{...s.card,marginBottom:14,background:"linear-gradient(135deg,#fff7ed,#fef3c7)",border:"1.5px solid #fcd34d"}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <div style={{fontSize:38}}>ðŸ”¥</div>
              <div style={{flex:1}}>
                <div style={{fontSize:20,fontWeight:800,color:"#d97706"}}>{streakCount} semaine{streakCount>1?"s":""} de suite !</div>
                <div style={{fontSize:12,color:"#92400e",marginTop:2}}>Planning complet ðŸŽ‰</div>
              </div>
            </div>
          </div>}
          {/* BADGES */}
          {(()=>{
            ...
            const BADGES=[
              {id:"cook",icon:"ðŸ…",label:"Cuisinier du mois",desc:"20 repas planifiÃ©s",unlocked:totalPlanned>=20},
              {id:"adv",icon:"ðŸŒŸ",label:"Aventurier",desc:"5 idÃ©es testÃ©es",unlocked:ideasTested>=5},
              {id:"fire",icon:"ðŸ”¥",label:"En feu",desc:"10 semaines de suite",unlocked:streakCount>=10},
              {id:"gourmet",icon:"â­",label:"Gourmet",desc:"10 favoris",unlocked:favCount>=10},
              {id:"balance",icon:"ðŸŒˆ",label:"Ã‰quilibrÃ©",desc:"50 repas planifiÃ©s",unlocked:totalPlanned>=50},
              {id:"legend",icon:"ðŸ’Ž",label:"LÃ©gende",desc:"50 plats enregistrÃ©s",unlocked:dishes.length>=50},
            ];
            const unlocked=BADGES.filter(b=>b.unlocked);
            const locked=BADGES.filter(b=>!b.unlocked);
            return <div style={{...s.card,marginBottom:14}}>
              <div style={{fontWeight:700,fontSize:13,color:T.text,marginBottom:10}}>ðŸ† Badges</div>
        ...
        {tab==="tools"&&<div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{fontWeight:800,fontSize:16,color:T.text,marginBottom:4}}>ðŸ”§ Outils & RÃ©glages</div>
          ...
          <div style={{...s.card}}>
            ...
            <div style={{textAlign:"center"}}>
              ...
              {timerSeconds===0&&<div style={{marginTop:12,padding:"10px",background:"#fbeaea",borderRadius:10,color:"#e05c6a",fontWeight:700,fontSize:13}}>{"ðŸ”” C'est prÃªt !"}</div>}
            </div>
          </div>

          {/* â”€â”€ THÃˆMES â”€â”€ */}
          <div style={{...s.card}}>
            <div style={{fontWeight:700,fontSize:13,color:T.text,marginBottom:12}}>ðŸŽ¨ ThÃ¨me couleur</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
              {Object.entries(PALETTES).map(([key,p])=>(
                <button key={key} onClick={()=>setPalette(key)} style={{
                  ...
                  {palette===key&&<div style={{position:"absolute",top:5,right:5,fontSize:12,color:T.accent,fontWeight:800}}>âœ“</div>}
                </button>
              ))}
            </div>
            <div style={{marginTop:12,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",borderRadius:12,background:T.bg,border:`1.5px solid ${T.cardBorder}`}}>
              <div>
                <div style={{fontSize:13,fontWeight:700,color:T.text}}>ðŸŒ™ Mode sombre</div>
          ...
          {(()=>{
            ...
            if(commonDishes.length<3) return <div style={{...s.card}}>
              <div style={{fontWeight:700,fontSize:13,color:T.text,marginBottom:6}}>ðŸ’‘ CompatibilitÃ© des goÃ»ts</div>
            ...
            return <div style={{...s.card}}>
              <div style={{fontWeight:700,fontSize:13,color:T.text,marginBottom:14}}>ðŸ’‘ CompatibilitÃ© des goÃ»ts</div>
              <div style={{textAlign:"center",marginBottom:14}}>
                ...
                <div style={{fontSize:12,color:T.textMuted}}>{compat>=80?"Vous avez des goÃ»ts trÃ¨s proches ðŸ’š":compat>=60?"Quelques diffÃ©rences, mais Ã§a se complÃ¨te ðŸ‘":"Vos goÃ»ts divergent souvent ðŸ¤”"}</div>
              ...
              {agree.length>0&&<div style={{marginBottom:10}}>
                <div style={{fontSize:10,fontWeight:800,color:T.green,textTransform:"uppercase",letterSpacing:0.8,marginBottom:6}}>ðŸ’š En accord</div>
                {agree.map(({dish})=><div key={dish.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                  <div style={{width:28,height:28,borderRadius:7,background:T.accentLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,overflow:"hidden",flexShrink:0}}>{(dish.thumbnail||dish.photo)?<img src={dish.thumbnail||dish.photo} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"ðŸ½ï¸"}</div>
              ...
              {disagree.length>0&&<div>
                <div style={{fontSize:10,fontWeight:800,color:"#e05c6a",textTransform:"uppercase",letterSpacing:0.8,marginBottom:6}}>ðŸ¤” DÃ©saccords</div>
        ...
        {/* â•â• EXPORT / RESTAURATION â•â• */}
        <div style={{...s.card,marginTop:12}}>
          <div style={{fontWeight:700,fontSize:13,color:T.text,marginBottom:12}}>{"ðŸ’¾ Sauvegarde & Restauration"}</div>
          <p style={{fontSize:12,color:T.textMuted,marginBottom:14,lineHeight:1.5}}>{"Exporte toutes tes donnÃ©es (plats, planning, idÃ©es) dans un fichier JSON. Tu pourras les restaurer plus tard."}</p>
          <button onClick={exportData} style={{...s.primary,width:"100%",marginBottom:10}}>{"â¬‡ï¸ Exporter mes donnÃ©es"}</button>
          <div style={{position:"relative"}}>
            <button style={{...s.ghost,width:"100%",opacity:restoring?0.6:1,pointerEvents:restoring?"none":"auto"}}>
              {restoring?"â³ Restauration en cours...":"â¬†ï¸ Restaurer depuis un fichier"}
            </button>
          ...
          <p style={{fontSize:11,color:T.textLight,marginTop:10,lineHeight:1.4}}>{"âš ï¸ La restauration Ã©crase les donnÃ©es existantes."}</p>
        </div>
      </div>
      ...
      {showConfetti&&<div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:9999,overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",top:"30%",left:"50%",transform:"translate(-50%,-50%)",textAlign:"center",
          ...
          <div style={{fontSize:44,marginBottom:6}}>ðŸŽ‰</div>
          <div style={{fontSize:18,fontWeight:800,color:"#1e2d3d"}}>Semaine complÃ¨te !</div>
          {streakCount>1&&<div style={{fontSize:13,color:"#d97706",fontWeight:700,marginTop:6}}>ðŸ”¥ {streakCount} semaines de suite !</div>}
        </div>
      </div>}
            ...
            <div style={{background:T.card,borderRadius:"24px 24px 0 0",width:"100%",maxWidth:480,maxHeight:"92vh",overflowY:"auto",overscrollBehavior:"contain",WebkitOverflowScrolling:"touch",boxShadow:`0 -20px 60px ${T.shadowMd}`}} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{position:"relative",width:"100%",flexShrink:0,background:hasPhoto?"#111":T.headerBg,overflow:"hidden"}}>
                ...
                {hasPhoto
                  ...
                  : <div style={{height:100,display:"flex",alignItems:"center",justifyContent:"center",fontSize:56,position:"relative",zIndex:2}}>ðŸ½ï¸</div>
                }
              ...
              <div style={{padding:"18px 18px 28px",display:"flex",flexDirection:"column",gap:16}}>
                ...
                <div style={{display:"flex",gap:20}}>
                  <div><div style={{fontSize:11,color:T.textMuted,marginBottom:4}}>Vaisselle</div><StarRating icon="ðŸ«§" value={d.dishesRating||0} max={3} color={T.accent} size={16}/></div>
                  <div><div style={{fontSize:11,color:T.textMuted,marginBottom:4}}>Temps</div><StarRating icon="â±ï¸" value={d.timeRating||0} max={3} color={T.green} size={16}/></div>
                </div>
                ...
                {links.filter(l=>l.url).length>0&&<div><div style={{fontSize:11,fontWeight:700,color:T.textMuted,textTransform:"uppercase",letterSpacing:0.5,marginBottom:6}}>Liens</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{links.filter(l=>l.url).map((l,i)=><a key={i} href={l.url} target="_blank" rel="noreferrer" style={{fontSize:13,color:T.accent,display:"flex",alignItems:"center",gap:6}}>ðŸ”— {l.label||l.url}</a>)}</div></div>}
                ...
                <div style={{display:"flex",gap:8}}>
                  <button onClick={()=>setConfirmAction({title:"Modifier ce plat ?",message:`Vos modifications remplaceront les donnÃ©es actuelles de "${d.name}".`,onConfirm:()=>{setEditDish(d);setViewDish(null);setConfirmAction(null);}})} style={{...s.ghost,flex:1}}>âœï¸ Modifier</button>
                  <button onClick={()=>{setPendingDishForPlan(d);setPlanSlot("__pick__");setSelectedSlots([]);setViewDish(null);}} className="btn-anim" style={{...s.primary,flex:1}}>ðŸ“… Planifier</button>
                </div>
                <button onClick={()=>setConfirmAction({title:"Supprimer ce plat ?",message:`"${d.name}" sera dÃ©finitivement supprimÃ©.`,onConfirm:()=>{deleteDish(d.id,d.name);setViewDish(null);setConfirmAction(null);}})} style={{...s.ghost,width:"100%",color:T.danger,borderColor:T.danger}}>ðŸ—‘ï¸ Supprimer ce plat</button>
              </div>
            </div>
          </div>
        );
      ...
      {confirmResetPlan&&<Modal title="Remettre Ã  zÃ©ro ?" onClose={()=>setConfirmResetPlan(false)}>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={resetPlanning} style={{...s.danger,flex:1}}>ðŸ—‘ï¸ Remettre Ã  zÃ©ro</button>
        </div>
      </Modal>}
      ...
      {historyWeek&&<Modal title={`Semaine du ${historyWeek}`} onClose={()=>setHistoryWeek(null)}>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {ALL_SLOTS.map(slot=>{const entry=weekPlans[historyWeek]?.[slot];if(!entry)return null;return <div key={slot} style={{...s.card,display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:10,background:T.accentLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,overflow:"hidden",flexShrink:0}}>{(entry.thumbnail||entry.photo)?<img src={entry.thumbnail||entry.photo} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"ðŸ½ï¸"}</div><div><div style={{fontSize:11,color:T.textMuted}}>{slot}</div><div style={{fontSize:14,fontWeight:700,color:T.text}}>{entry.name}</div></div></div>;})}
        ...
        {tab==="elodieDishes"&&<div>
          <div style={{...s.card,marginBottom:12,background:"linear-gradient(135deg,#f3e8ff,#ede9fe)",border:"1.5px solid #c4b5fd"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <div style={{fontSize:28}}>ðŸ‘©â€ðŸ³</div>
              <div>
                <div style={{fontSize:13,fontWeight:800,color:"#7c3aed"}}>Plats perso â€” Ã‰lodie</div>
                <div style={{fontSize:11,color:"#8b5cf6",marginTop:2}}>{isElodie?"Visible uniquement par toi ðŸ’œ":"Consultation uniquement ðŸ‘€"}</div>
              </div>
            </div>
          </div>
          <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center"}}>
            <button onClick={()=>setElodieSearchQ(q=>q===null?"":null)} style={{width:42,height:42,borderRadius:11,border:`1.5px solid ${elodieSearchQ!==null?T.accent:T.inputBorder}`,background:elodieSearchQ!==null?T.accentLight:T.card,cursor:"pointer",fontSize:19,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>ðŸ”</button>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
            <button onClick={()=>setElodieFilterFav(f=>!f)} style={{fontSize:12,padding:"5px 12px",borderRadius:10,cursor:"pointer",border:`1.5px solid ${elodieFilterFav?"#f59e0b":T.inputBorder}`,background:elodieFilterFav?"#fef3c7":"transparent",color:elodieFilterFav?"#d97706":T.textMuted,fontFamily:"inherit",fontWeight:elodieFilterFav?700:400}}>â˜… Favoris</button>
          ...
          {elodieFiltered.length===0&&<div style={{textAlign:"center",color:T.textLight,padding:"40px 0"}}>Aucun plat perso ðŸ½ï¸<br/><span style={{fontSize:11}}>Ajoute tes plats rien qu'Ã  toi !</span></div>}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        ...
        {/* â•â• PLANNING Ã‰LODIE â•â• */}
        {tab==="elodiePlan"&&isElodie&&<div>
          <div style={{...s.card,marginBottom:12,background:"linear-gradient(135deg,#f3e8ff,#ede9fe)",border:"1.5px solid #c4b5fd"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <div style={{fontSize:28}}>ðŸ’œ</div>
              <div>
                <div style={{fontSize:13,fontWeight:800,color:"#7c3aed"}}>Planning perso â€” Ã‰lodie</div>
                <div style={{fontSize:11,color:"#8b5cf6",marginTop:2}}>Visible uniquement par toi ðŸ’œ</div>
              </div>
            </div>
          </div>
          <div style={{display:"flex",gap:8,marginBottom:8,alignItems:"center"}}>
            <div style={{display:"flex",background:T.segBg,borderRadius:12,padding:3,flex:1,gap:3}}>
              {[{id:"weekday",label:"Lun â†’ Ven midi",icon:"ðŸ’¼"},{id:"weekend",label:"Ven soir â†’ Dim",icon:"ðŸŒ¿"}].map(v=><button key={v.id} onClick={()=>setElodiePlanView(v.id)} style={{flex:1,padding:"9px 6px",borderRadius:9,border:"none",background:elodiePlanView===v.id?T.card:"transparent",color:elodiePlanView===v.id?(v.id==="weekday"?"#7c3aed":T.weekendHeader):T.textMuted,fontWeight:elodiePlanView===v.id?700:400,fontSize:12,cursor:"pointer",fontFamily:"inherit",boxShadow:elodiePlanView===v.id?`0 2px 8px ${T.shadow}`:"none"}}>{v.icon} {v.label}</button>)}
            </div>
            <button onClick={()=>setElodieConfirmReset(true)} title="Remettre Ã  zÃ©ro" style={{background:T.card,border:`1.5px solid ${T.inputBorder}`,borderRadius:10,padding:"8px 10px",cursor:"pointer",fontSize:16,color:T.danger,flexShrink:0}}>ðŸ—‘ï¸</button>
          </div>
          {/* Toggle source plats Ã‰lodie */}
          <div style={{display:"flex",background:T.segBg,borderRadius:10,padding:2,gap:2,marginBottom:14}}>
            {[{id:"own",label:"ðŸ‘©â€ðŸ³ Mes plats"},{id:"other",label:"ðŸ§‘â€ðŸ³ Plats ThÃ©o"}].map(opt=>(
          ...
          {elodiePastWeeks.length>0&&<div style={{marginTop:24}}><div style={{fontWeight:700,color:T.textMuted,fontSize:11,textTransform:"uppercase",letterSpacing:0.5,marginBottom:10}}>ðŸ“š Semaines passÃ©es</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{elodiePastWeeks.map(wk=><button key={wk} onClick={()=>setElodieHistoryWeek(wk)} style={{...s.card,padding:"10px 14px",textAlign:"left",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontWeight:600,color:T.text,fontSize:13}}>Semaine du {wk}</div><div style={{fontSize:11,color:T.textMuted}}>{Object.values(elodieWeekPlans[wk]||{}).filter(Boolean).length} repas</div></button>)}</div></div>}
        </div>}

      {/* â•â• MODALES Ã‰LODIE â•â• */}

      {/* Modale ajout/edit plat Ã‰lodie */}
      ...
      {elodieViewDish&&<Modal title={elodieViewDish.name} onClose={()=>setElodieViewDish(null)}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
          ...
          {elodieViewDish.favorite&&<span style={{background:"#fef3c7",color:"#d97706",borderRadius:8,padding:"3px 10px",fontSize:12,fontWeight:700}}>â˜… Favori</span>}
        </div>
        {elodieViewDish.notes&&<p style={{fontSize:13,color:T.textMuted,marginBottom:14,lineHeight:1.6}}>{elodieViewDish.notes}</p>}
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          <button onClick={()=>{setElodieEditDish(elodieViewDish);setElodieViewDish(null);}} style={{...s.ghost,flex:1}}>âœï¸ Modifier</button>
          <button onClick={()=>toggleElodieFav(elodieViewDish)} style={{...s.ghost,flex:1}}>{elodieViewDish.favorite?"â˜… Retirer favori":"â˜† Favori"}</button>
        </div>
        <button onClick={()=>deleteElodieDish(elodieViewDish.id)} style={{...s.ghost,width:"100%",color:T.danger,borderColor:T.danger}}>ðŸ—‘ï¸ Supprimer ce plat</button>
      </Modal>}

      {/* PlanPick Ã‰lodie â€” choisir un crÃ©neau */}
      {elodiePlanSlot==="__pick__"&&elodiePendingDish&&<Modal title={`Planifier "${elodiePendingDish.name}"`} onClose={()=>{setElodiePlanSlot(null);setElodiePendingDish(null);setElodieSelectedSlots([]);}}>
        <div style={{fontSize:12,color:T.textMuted,marginBottom:10}}>SÃ©lectionne un ou plusieurs crÃ©neaux :</div>
        ...
        <button disabled={elodieSelectedSlots.length===0} onClick={()=>assignElodieDish(elodiePendingDish,elodieSelectedSlots)} style={{...s.primary,width:"100%",background:"#7c3aed",opacity:elodieSelectedSlots.length===0?0.5:1}}>
          {elodieSelectedSlots.length===0?"SÃ©lectionne un crÃ©neau":`Planifier (${elodieSelectedSlots.length} crÃ©neau${elodieSelectedSlots.length>1?"x":""})`}
        </button>
      </Modal>}

      {/* Historique semaine Ã‰lodie */}
      {elodieHistoryWeek&&<Modal title={`Semaine du ${elodieHistoryWeek}`} onClose={()=>setElodieHistoryWeek(null)}>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {ALL_SLOTS.map(slot=>{const entry=elodieWeekPlans[elodieHistoryWeek]?.[slot];if(!entry)return null;return <div key={slot} style={{...s.card,display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:10,background:T.accentLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,overflow:"hidden",flexShrink:0}}>{entry.photo?<img src={entry.photo} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"ðŸ½ï¸"}</div><div><div style={{fontSize:11,color:T.textMuted}}>{slot}</div><div style={{fontSize:14,fontWeight:700,color:T.text}}>{entry.name}</div></div></div>;})}
          {ALL_SLOTS.every(x=>!elodieWeekPlans[elodieHistoryWeek]?.[x])&&<div style={{textAlign:"center",color:T.textLight,padding:"24px 0"}}>Aucun repas planifiÃ©</div>}
        </div>
      </Modal>}

      {/* Confirm reset planning Ã‰lodie */}
      {elodieConfirmReset&&<Modal title="Remettre Ã  zÃ©ro ?" onClose={()=>setElodieConfirmReset(false)}>
        <p style={{fontSize:13,color:T.textMuted,marginBottom:16}}>Tous les repas de ta semaine perso seront effacÃ©s.</p>
        <div style={{display:"flex",gap:8}}>
