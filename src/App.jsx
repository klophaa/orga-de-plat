...
// + Planning : miniatures agrandies (48×48px)
        ...
        {display?(
          <div draggable onDragStart={()=>setDragItem({slot,dish:entry})}>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <div style={{width:48,height:48,borderRadius:10,background:T.accentLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,overflow:"hidden",flexShrink:0}}>
      ...
      {planSlot&&planSlot!=="__pick__"&&<Modal title={`Choisir pour : ${planSlot}`} onClose={()=>{setPlanSlot(null);setPlanSearch("");setPlanFreeText("");}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          <input
            ...
            onKeyDown={e=>{if(e.key==="Enter"&&planFreeText.trim()){assignDish({id:null,name:planFreeText.trim(),photo:null,thumbnail:null},planSlot);setPlanFreeText("");setPlanSearch("");}}}
          />
          <button
            onClick={()=>{if(planFreeText.trim()){assignDish({id:null,name:planFreeText.trim(),photo:null,thumbnail:null},planSlot);setPlanFreeText("");setPlanSearch("");}}}
            disabled={!planFreeText.trim()}
            style={{...s.primary,padding:"10px 14px",flexShrink:0,opacity:planFreeText.trim()?1:0.4}}
          >✓</button>
        </div>
        {/* Séparateur */}
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
          <div style={{flex:1,height:1,background:T.cardBorder}}/>
          <span style={{fontSize:11,color:T.textMuted,fontWeight:600}}>ou choisir une fiche</span>
          <div style={{flex:1,height:1,background:T.cardBorder}}/>
        </div>
        {/* Recherche parmi les fiches — input non-contrôlé pour éviter perte de focus */}
        <input
          key={planSlot}
          autoFocus
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:320,overflowY:"auto"}}>
          ...
          {dishes.filter(d=>!planSearch||d.name.toLowerCase().includes(planSearch.toLowerCase())).length===0&&<div style={{textAlign:"center",color:T.textLight,padding:"20px 0",fontSize:13}}>Aucun plat {planSearch ? "trouvé" : ""}</div>}
        </div>
      </Modal>}
